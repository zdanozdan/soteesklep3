 ...
 * @version $Id: ConvertCharset.class.php,v 1.3 2005/04/28 11:59:19 lechu Exp $
...
class ConvertCharset {
	...
	function Convert ($StringToChange, $FromCharset, $ToCharset, $TurnOnEntities = false)
	{
		 ...
		 **/
		if(strpos($FromCharset, '|') !== false) {
    		$tmp_arr = explode('|', $FromCharset);
    		$FromCharset = $tmp_arr[1];
		}
		$FromCharset = strtolower($FromCharset);
		if(strpos($ToCharset, '|') !== false) {
    		$tmp_arr = explode('|', $ToCharset);
    		$ToCharset = $tmp_arr[1];
		}
