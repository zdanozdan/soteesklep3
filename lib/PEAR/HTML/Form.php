...
*
* @depend      include/form_check.inc
*
* @modified_by m@sote.pl
* @version     $Id: Form.php,v 1.14 2005/10/21 10:08:41 lechu Exp $
* @package     soteesklep
*/
...
// +----------------------------------------------------------------------+
//
// $Id: Form.php,v 1.14 2005/10/21 10:08:41 lechu Exp $
//
...
if (!defined('HTML_FORM_PASSWD_SIZE')) {
    define('HTML_FORM_PASSWD_SIZE', 16);
}
...
class HTML_Form
{
    ...
    function HTML_Form($action, $method = 'get', $name = '', $target = '', $enctype = '')
    {
        ...
        // start m@sote.pl:
        global $form_check;
        if (! empty($form_check)) $this->form_check=&$form_check;
        // end   m@sote.pl:
    ...
    // {{{ addText()
    /**
    * @modified_by m@sote.pl add param $attrib
    */
    function addText($name, $title, $default = '',
    $size = HTML_FORM_TEXT_SIZE, $maxlength = '', $attrib = '')
    {
        $this->fields[] = array("text", $name, $title, $default, $size, $maxlength, $attrib);
    }
    ...
    function addImage($name, $text, $src, $script_css, $position)
    {
        $this->fields[] = array("image", $name, $text, $src, $script_css, $position);
    }
    ...
    /**
    * Modified_by m@sote.pl dele basename($this->action) -> add ($this->action)
    */
    function start()
    {
...
//        $this->displayError($name);
    ...
    */
    ...
    $size = HTML_FORM_TEXT_SIZE, $maxlength = '', $attrib = '')
    {
        ...
        print "  <td>";
        $this->displayText($name, $default, $size, $maxlength, $attrib);
        print "</td>\n";
        print " </tr>\n";
    }
...
//        $this->displayError($name);
    ...
    // }}}
    // {{{ displayImageRow()
    function displayImageRow($name, $text, $src, $script_css, $position)
    {
    	  if($position == 'first') {
	          print "<tr>\n";
	          print "	<td>&nbsp;</td><td>";
    	  }
        echo "        $text <input type=\"image\" src=\"$src\" name=\"$name\" $script_css>&nbsp;";
    	  if($position == 'last') {
              print "	</td>\n";
              print "</tr>\n";
    	  }
    ...
    // start SOTE
    // @modified_by m@sote.pl
    function displayCheckbox($name, $default = false, $value='')
    {
        print "<input type=\"checkbox\" name=\"$name\"";
        if (! empty($value)) print " value=\"$value\"";
        // end SOTE
...
//            $this->displayError($name);
    ...
    {
        print " <tr>\n";
        // SOTE print "  <td align=\"right\">$title</td>";
        print "  <td></td>";
        // end SOTE
        print "  <td>";
        print "  <nobr>";
...
//        $this->displayError($name);
    ...
    /**
    * Wyswietl informacja o bledzie, bledne wypelnienie pola formularza
    *
    * @author m@sote.pl
    *
    ...
    function displayError($name) {
        ...
        if (! empty($this->form_check)) {
            if (! empty($this->form_check->errors_found[$name])) {
                print "<font color=red>";print $this->form_check->errors_found[$name];print "</font>\n";
            }
        }
        return (0);
    } // end displayError()
