...
class tar_file extends archive
{
	...
	function extract_files($flat=true,$prefix='')
	{
		$pwd = getcwd();
		chdir($this->options['basedir']);
		...
		if ($fp = $this->open_archive())
		{
			...
			while ($block = fread($fp, 512))
			{
				...
				// i dont want directories _and_ i _always_ want to know what file was written where!
				if ($flat) {
					$old_name=$file['name'];
					$unique=md5(uniqid('',true));
					$file['name']=$unique."_".basename($file['name']);
				}
				$file['name']=$prefix.$file['name'];
				if ($file['type']!=5) {	
					$this->filestruct[$file['name']]=$old_name;
				}
				if ($this->options['inmemory'] == 1)
				{
				...
				{
					// no fucking subdirectories
					/*if (!is_dir($file['name']) && !$flat)
