...
*
* @(#) $Header: /usr/local/CVS/soteesklep2/lib/Metabase/metabase_database.php,v 2.4 2005/09/02 08:08:00 lukasz Exp $
*
*/
...
class metabase_database_class
{
    ...
    Function Error()
    {
        // sote - dodanie automatycznego naprawienia tabeli w bazie danych        
        if (ereg("Can't open file",$this->last_error)) {
            preg_match("/Can't open file: '(.*).MYD'/",$this->last_error,$matches);
            if (! empty($matches[1])) $table=$matches[1];
            if (! empty($table)) {
                global $_SESSION;
                if (@$_SESSION['__db_repair_table']!=$table) {
                    $this->query("REPAIR TABLE $table");
                    global $theme;
                    $theme->go2main(3);                    
                    $this->last_error.="<p />I'm trying to repair database. Please wait...<p />\n";
                    global $sess;
                    global $__db_repair_table;
                    $__db_repair_table=$table;
                    $sess->register("__db_repair_table",$db_repair_table);
                }
            }
        }
        // end sote
    ...
    {
        return($this->SetError("Create sequence","sequence creation is not supported"));
    }

    Function DropSequence($name)
    {
        return($this->SetError("Drop sequence","sequence dropping is not supported"));
    }
    ...
    {
        return($this->SetError("Get sequence current value","getting sequence current value is not supported"));
    }
