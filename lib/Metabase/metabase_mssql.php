 ...
 * metabase_mssql.php
 *
 * @(#) $Header: /usr/local/CVS/soteesklep2/lib/Metabase/metabase_mssql.php,v 2.1 2003/03/13 11:48:25 maroslaw Exp $
 *
 */
...
class metabase_mssql_class extends metabase_database_class
{
	...
	Function Query($query)
	{
		...
		if(!$this->SelectDatabase())
			return(0);
		$query_command=strtolower(ltrim($query));
		if(($select=(substr($query_command,0,strlen("select"))=="select"))
		&& $limit>0)
		{
			$result=0;
