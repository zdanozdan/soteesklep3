 ...
 * @version $Id: load.inc.php,v 1.1 2005/12/22 11:41:23 scalak Exp $
* @package    offline
* @subpackage main
 */
...
require_once("include/depository.inc");
...
class OfflineLoad {
    ...
    var $_newEncoding='';			// klasa do konwersji langow.
	...
	*/
	function OfflineLoad() {
		...
		$deliverers=$database->sql_select_array("id","deliverers","");
		...
		foreach($config->offline_relation as $key => $value) {
			foreach($this->record_hash as $key1 => $value1) {
				if($value1 != '') {
					...
					if(!empty($code)) {
						$value1 = $this->_newEncoding->Convert($value1, "utf-8", $code, 0);
					}	
					...
					if( $key == $key1 ) {
						if((ereg("category[12345]",$key) || $key == 'producer')) {
	                        ...
	                        //print $value1;
						    if(preg_match("/[p]/",$value1)) {
	                           $value1=preg_replace("/p/","",$value1);
						       $data = $data+array($key=>"num+".$value1);
	                        } elseif(preg_match("/[m]/",$value1)) {
	                           $value1=preg_replace("/m/","",$value1);
						       $data = $data+array($key=>"num-".$value1);
	                        } else {
                                $data = $data+array($key=>$value1);
	                          }
	...
	*/
	function update_data() {
		...
		if($exist_key > 0 ) {
		    $data1=$data;
		    unset($data['user_id_main']);
		    $result=$database->sql_update2($this->table,"$this->field_sql=$key",$data);
			$this->update_main($data1);
		    $this->record_update++;
			return true;
		} else {
			$result=$this->insert_data();
	...
	*/
	function insert_data() {
		...
		$data['num']=preg_replace("/num/","",$data['num']);
		$result=$database->sql_insert($this->table,$data);
		$this->update_main($data);
		if($result == 0 ) return false;
	...
	*/
	...
	/**
	 * Funkcja aktualizuje pole id_available w tablicy main.
	 *
	 * @param unknown $data
	 */
    ...
    {
    	global $database;
    	$count=$database->sql_select("num","depository","user_id_main=".$data['user_id_main']);
    	$dep=new Depository;
    	$count_dep=$dep->compute($count,$data['user_id_main'],$data['id_deliverer']);
        $database->sql_update("main","user_id=".$data['user_id_main'], array("id_available"=>$count_dep));
    } // end func update_main
