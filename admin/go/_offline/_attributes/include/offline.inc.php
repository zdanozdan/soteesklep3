 ...
 * @version $Id: offline.inc.php,v 1.4 2004/12/20 17:58:24 maroslaw Exp $
* @package    offline
...
$count=count($record);
if($count == 0 ) {
	$error->write_other($lang->offline_file_errors['file_error'],"&nbsp;");
	$status=false;
}
for ($i=0;$i<$count;$i++) {
    if ($i>$last_num) {
...
$cancel=$category->load_attributes();
if($cancel > 0) {
	$status=false;
}
...
print $lang->offline_record_added." - ".($load->record_add - $cancel)."<br>";
