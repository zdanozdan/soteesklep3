...
* @version $Id: load.inc.php,v 1.6 2004/12/20 17:58:24 maroslaw Exp $
* @package    offline
...
class OfflineLoad {
	...
	var $field_code='';
	var $check_record='';            // czy bedziemy sprawdzac czy rekord istnieje w bazie przed wlozeniem go do bazy danych.
	var $id='';						// pole to bedzie zawieralo aktualne id atrybutu
	...
	*/
	...
	function OfflineLoad() {
		...
		$this->check_record=$config->offline_check_record;
	...
	function check_record($record,$number) {
		...
		if($form_check->form_test()) {
			...
			foreach($form_check->errors_found as $key => $value ) {
				if ($number!=0) {   // wykluczamy pierwszy rekord - nazwy atrybutow
	...
	*/
	function load_record($record,$number) {
		...
		if($this->count_record < $this->count_limit) {
			...
			} elseif ($number==0) {              // pierwszy wiersz nie ma przechodzic przez walidacje
			$result = $this->insert_data();
			if(!$result) {
				$error->write_other($lang->offline_file_errors['error_insert'],$number);
			} else {
	...
	*/
	...
	function delete_table() {
		global $database;
		$database->sql_delete($this->table);
		$database->sql_delete("attributes_names");
		return true;
	}
...
//		print "<br>id=".$this->id."<br>";
		// czy ladowanie rekordów ze sprawdzaniem ?
		if($this->check_record == 'true') {
			$key=$this->record_hash[$this->field_sql];
			$exist_key=$database->sql_select("count(*)",$this->table,"$this->field_sql=$key");
			if($exist_key > 0 ) {
...
//				$this->record_add++;
			...
			// zaladuj nazwy atrybutow do bazy
//			print $data['id_main']."=".$this->id;
			if ($data['id_main']!=$this->id) {
				$this->id=$data['id_main'];
				$result=$database->sql_insert("attributes_names",$data);
				$this->record_add++;
				return true;
			} else {
				$result=$database->sql_insert($this->table,$data);
//				$this->record_add++;
	...
	*/
	...
	function delete_data() {
		...
		$key=$this->record_hash[$this->field_sql];
		// czy kasowanie rekordów ze sprawdzaniem ?
		if($this->check_record == 'true') {
			$exist_key=$database->sql_select("count(*)",$this->table,"$this->field_sql=$key");
			if($exist_key > 0 ) {
