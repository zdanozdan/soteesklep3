<?php
/**
* Edycja parametrów produktu wymaganych do exportu do pasaz.interia.pl
*
* @author  rdiak@sote.pl
* @version $Id: edit_interia.php,v 2.1 2005/03/29 15:17:12 scalak Exp $
*
...
include_once ("include/date_manage.inc.php");
...
if ($update==true) {
	...
	//$form_check->fun['interia_export']="int";
	//$form_check->fun['interia_status']="int";
	
	$form_check->form=$_REQUEST['item'];
	$form_check->errors=$lang->interia_form_errors;
	...
	if ($form_check->check=="true") {
		if ($form_check->form_test()) {
			// poprawnie interiarowadzone dane, aktualizuj tylko przy edycji
			if (@$__add!=true) {
				require_once ("./include/edit_update_interia.inc.php"); // aktualizuj parametrow interia, zalacz zdjecia htdocs/photo
			}
			...
			$action="edit_interia.php";
			require_once("./html/edit_interia.html.php");
...
if ($prepared_query) {
    ...
    if ($result!=0) {
        ...
        if ($num_rows>0) {
            require_once("include/query_rec_interia.inc.php");
