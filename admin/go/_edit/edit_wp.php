<?php
/**
* Edycja parametrów produktu wymaganych do exportu do pasaz.wp.pl
*
* @author  rdiak@sote.pl
* @version $Id: edit_wp.php,v 2.6 2005/01/20 14:59:20 maroslaw Exp $
*
...
if ($update==true) {
	...
	//$form_check->fun['wp_export']="int";
	//$form_check->fun['wp_status']="int";
	
	$form_check->form=$_REQUEST['item'];
	$form_check->errors=$lang->wp_form_errors;
	...
	if ($form_check->check=="true") {
		if ($form_check->form_test()) {
			...
			if (@$__add!=true) {
				require_once ("./include/edit_update_wp.inc.php"); // aktualizuj parametrow wp, zalacz zdjecia htdocs/photo
			}
			...
			$action="edit_wp.php";
			require_once("./html/edit_wp.html.php");
...
if ($prepared_query) {
    ...
    if ($result!=0) {
        ...
        if ($num_rows>0) {
            require_once("include/query_rec_wp.inc.php");
