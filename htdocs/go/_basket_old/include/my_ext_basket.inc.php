...
//				$form['points_left']=$form['points_left']-$points_value;
    ...
    */
	function showBasketForm() {
		...
		// button kontynuuj zakupy
		if ($this->display=='form') {
                   $theme->back_button($lang->basket_order_continue);
                   if (strcmp($formname,"wishlistForm") != 0)
                      $theme->display3steps(1);
		}
                // TZ end
                ///////////////////////////////////////////////////
...
//		print "</pre>";
	...
	function showBasketFormLimited() {
		...
		if ($this->display=="form") {
                   if ($this->_display_above) {
                      ...
                      if ($config->ssl) {
                         ...
                         print "<form action=\"/go/_basket/index.php\" method=\"post\" name=\"$formname\">\n";
                      }
                   }
                   print "<input name=\"formName\" id=\"formName\" value=\"$formname\" type=hidden>\n";
		}
		...
		print  "<table border=\"0\" cellspacing=\"1\" cellpadding=\"0\" align=\"center\" width=\"100%\">\n";
    ...
    */
	function _rowLimited($id,$item) {
		...
		print "<tr bgcolor=" . @$config->theme_config['colors']['basket_td'] . ">\n";
		print "<td style=\"text-align:left\">".$namelink;
		if (! empty($item['data']['options'])) {
			$options=$item['data']['options'];
			$this->_showOptions($options,"html");
		}
		...
		if ($this->display=="form") {
            ...
            <input type=\"text\" size=\"4\" maxsize=\"6\" name=\"num[$id]\" value=\"".$item['num']."\">";
                        print "&nbsp";
                        //<input type=\"submit\" name=\"submit_update\" value=\"order_update\">";
                        $theme->calc_buttonLimited();
			if ($_SESSION['depository_status'] == false and isset($_SESSION['depository_status'])) {
				for ($i=0;$i<=count($num_id)-1;$i++)
				{
					if ($user_id == $num_id[$i] and $num_max != 0){ print "<br /><font color=\"red\"><b>($num_max $lang->basket_max)</b></font>"; break; } 
		...
		if ($this->display=="form") {
			...
			if ($this->mode!='points') {
				...
				//onClick=\"this.form.submit();
                        ...
                        print "<td align=\"center\"><input \"border0\" type=\"submit\" id=\"del\" value=\"$lang->delete\" name=\"del[$id]\" ></td>\n";
